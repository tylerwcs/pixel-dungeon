...
export async function handleLobbyApi(request,env={},options={}){
  ...
  if(action==='claim'){
    ...
    const character=await options.characterStorage?.get(input.characterId);if(!character)return json({error:'That character is no longer available.'},404);
    const guestHash=await hashToken(input.guestToken),previous=lobby.slots.find(candidate=>candidate.guestHash===guestHash);
