...
function renderPlayers(){
  const active=!lobby,inGameNames=playerNames(players);document.querySelectorAll('#players > *').forEach(element=>element.remove());
  players.forEach((player,index)=>{
    ...
    const hud=$(`hud${index}`);if(hud){const collector=index===state.collector;hud.classList.toggle('is-collector',collector);$(`hudName${index}`).textContent=inGameNames[index];$(`hudRole${index}`).textContent=collector?'★ COLLECTOR':'PURSUER';}
  });
...
function podiumCard(entry,place,compact=false){const detail=scoreDetails[entry.index],name=playerNames(players)[entry.index];return `<article class="podium-entry rank-${place}${compact?' podium-fourth':''}" style="--player:${COLORS[entry.index]}"><span class="podium-place">${place}</span><div class="podium-player"><canvas class="podium-portrait" data-podium-player="${entry.index}" width="220" height="220" aria-label="Player ${entry.index+1} character"></canvas><strong>${escapeHTML(name)}</strong><span>${entry.score} GOLD</span>${compact?'':`<small>${detail.collected} collected${detail.bonus?` · +${detail.bonus} catches`:''}</small>`}</div>${compact?'':`<div class="podium-step"><b>${place}</b></div>`}</article>`;}
...
function showPodium(){
  ...
  screen.innerHTML=`<div class="podium-panel"><img class="podium-event-logo" src="assets/yep-event-logo.webp" alt="Ecopiana Year End Party 2026"><div class="podium-confetti" aria-hidden="true">${confettiMarkup()}</div><header class="podium-heading"><span class="eyebrow">GAME COMPLETE · FINAL RANKINGS</span><h2 id="podiumTitle">${escapeHTML(playerNames(players)[winner.index].toUpperCase())} WINS!</h2></header><div class="podium-results"><div class="podium-stage"><div class="podium-top-three">${podiumCard(ranking[1],2)}${podiumCard(ranking[0],1)}${podiumCard(ranking[2],3)}</div>${podiumCard(ranking[3],4,true)}</div></div><footer class="podium-actions"><button id="playAgain" class="primary-button">PLAY AGAIN <span>▶</span></button><button id="podiumLobby" class="secondary-button">RETURN TO LOBBY</button></footer></div>`;
