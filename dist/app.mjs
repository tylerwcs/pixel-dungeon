...
const players=Array.from({length:4},(_,i)=>({control:i===0?'wasd':'ai',collector:null,pursuer:null}));
let state=newRound(players),lobby=true,muted=false,audio=null,pausedPhase='playing',pauseReason='',lastPhase='',lastCount=-1;
let pads=[],padSignature='',toastTimer;
const render=createRenderer($('game'));
let storageWarning=false;
async function savePreferences(){try{await writeSettings({version:1,muted,players:players.map(p=>({...p}))});}catch{if(!storageWarning){storageWarning=true;toast('Browser storage is unavailable. Changes will last for this visit only.');}}}
...
function renderPlayers(){
  ...
  $('players').replaceChildren();players.forEach((p,i)=>{
    ...
    card.innerHTML=`<div class="player-top"><canvas class="portrait" width="88" height="88" aria-label="Player ${i+1} character"></canvas><div class="player-info"><strong style="color:${COLORS[i]}">Player ${i+1}</strong><small>${i===state.collector?'✦ COIN COLLECTOR':p.control==='ai'?'AI PURSUER':'PURSUER'}</small></div><button class="edit-character" aria-label="Customize Player ${i+1}" title="Customize character">✎</button></div><select class="control-select" aria-label="Player ${i+1} controls"></select>`;
    const select=card.querySelector('select'),options=[['ai','Computer-controlled'],['wasd','Keyboard · WASD'],['arrows','Keyboard · Arrow keys'],...pads.map(g=>[`pad:${g.index}`,`Gamepad ${g.index+1}`])];
  ...
  $('startNote').textContent=humanCount()===0?'Choose a keyboard or gamepad to play.':!controlsValid()?'Reconnect the assigned gamepad to play.':`${humanCount()} human${humanCount()===1?'':'s'} + ${4-humanCount()} computer-controlled pursuer${4-humanCount()===1?'':'s'}`;
}
...
function frame(now){const dt=Math.min((now-last)/1000,.1);last=now;pollPads();if(!lobby){accumulator+=dt;while(accumulator>=1/60){step(state,1/60).forEach(beep);accumulator-=1/60;}}else accumulator=0;updateOverlay();render(state,players,now/1000);$('coins').textContent=state.coins.size;$('roundLabel').textContent=`ROUND ${String(state.round).padStart(2,'0')}`;$('statusLabel').textContent=lobby?'WAITING FOR PARTY':state.phase==='playing'?'CHASE IN PROGRESS':state.phase.toUpperCase();
  document.querySelectorAll('.portrait').forEach((c,i)=>{const ctx=c.getContext('2d');ctx.clearRect(0,0,c.width,c.height);const role=i===state.collector?'collector':'pursuer';drawSprite(ctx,players[i][role]||defaults[role],44,44,84,now/1000);});
...
try{const saved=await readSettings();if(saved?.version===1&&Array.isArray(saved.players)&&saved.players.length===4){const used=new Set();for(let i=0;i<4;i++){const p=saved.players[i],control=p?.control;if(typeof control==='string'&&(/^(wasd|arrows|ai)$/.test(control)||/^pad:\d+$/.test(control))&&(control==='ai'||!used.has(control))){players[i].control=control;used.add(control);}else players[i].control='ai';for(const role of ['collector','pursuer'])if(p?.[role]){try{const a=validateSettings(p[role]);if(typeof a.src!=='string'||(!a.src.startsWith('data:image/png;base64,')&&!Object.values(defaults).some(d=>d.src===a.src)))continue;await loadImage(a.src);players[i][role]=a;}catch{toast('A saved character could not load. Its default has been restored.');}}}if(!humanCount())players[0].control='wasd';muted=!!saved.muted;state=newRound(players);}}catch{storageWarning=true;toast('Browser storage is unavailable. You can still play and import characters for this visit.');}
updateSound();
