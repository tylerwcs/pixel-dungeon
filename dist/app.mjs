...
function renderPlayers(){
  ...
  players.forEach((player,index)=>{
    ...
    const characterName=open?'OPEN SLOT':characterLabel(player.booth||playerAsset(index,index===state.collector?'collector':'pursuer'));
    card.innerHTML=`<div class="player-card-head"><span class="player-badge">${index+1}</span></div><div class="player-card-body"><div class="character-tile"><canvas class="portrait" width="220" height="220" aria-label="Player ${index+1} character"></canvas><strong>PLAYER ${index+1}</strong></div><div class="player-summary"><div class="player-info"><strong>${escapeHTML(characterName)}</strong><small>${role}</small></div>${action}</div></div><button class="ready-button ${player.ready?'ready':''}" type="button">${open?'WAITING TO JOIN':ai?'◆ AI READY':player.ready?'✓ READY':'PRESS READY'}</button>`;
    $('players').append(card);
